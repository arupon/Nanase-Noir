Shader "SuperNekoya/OpenLitRKS"
{
    Properties
    {
        ...
        _Color2MatCapMask ("_Color2MatCapMask", 2D) = "white" {}
        _MatCapAtten ("_MatCapAtten", Float) = 1
        ...
        [Space]
        _CustomGravitySize ("Gravity", float) = 0.1
        _CustomPointA ("Point A", Vector) = (0, 0, 0, 0)
        _CustomPointB ("Point B", Vector) = (0, 1, 0, 0)
        _DPSOrificeChannel ("DPS Orifice Channel", int) = 1
    ...
    SubShader
    {
        ...
        // [RKS]
        float _CustomGravitySize;
        float4 _CustomPointA;
        float4 _CustomPointB;
        int _DPSOrificeChannel;
        ...
        Pass
        {
            ...
            struct v2f
            {
                ...
                nointerpolation float3 indirectLight : TEXCOORD11;
                UNITY_FOG_COORDS(6)
            ...
            v2f vert(appdata v)
            {
                ...
                float4 vertex = float4(v.vertex.xyz, 1);
                realkiss_vert(vertex, v.uv.y, _CustomGravitySize, _CustomPointA, _CustomPointB, _DPSOrificeChannel);
                o.positionWS = mul(unity_ObjectToWorld, vertex);
                o.pos = UnityWorldToClipPos(o.positionWS);
                o.uv = TRANSFORM_TEX(v.uv, _MainTex);
                o.normalWS = UnityObjectToWorldNormal(v.normalOS);
                o.normalSS = mul((float3x3)UNITY_MATRIX_V, o.normalWS);
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                // does not render if uv.x > .5(Dirty but works)
                if (i.uv.x > .5) discard;
                ...
                half4 col = lerp(_Color * tex2D(_MainTex, i.uv), _Color2, _MatCapAtten * tex2D(_Color2MatCapMask, i.normalSS.xy * 0.5 + 0.5).r * _Color.a);
